...
interface EditResultModalProps {
    ...
    result: EditRecord | null;
    onClose: () => void;
}

export function EditResultModal({ open, onOpenChange, isUpdating, progress, result, onClose }: EditResultModalProps) {
    const percentage = progress.total > 0 ? Math.round((progress.current / progress.total) * 100) : 0;
    const baseUrl = (import.meta.env.VITE_JIRA_URL || '').replace(/\/+$/, '');

    return (
        <Dialog open={open} onOpenChange={(val) => !isUpdating && onOpenChange(val)}>
            <DialogContent className="max-w-2xl" onInteractOutside={(e) => isUpdating && e.preventDefault()}>
                <DialogHeader>
                    <DialogTitle>{isUpdating ? 'Updating tickets...' : 'Edit result'}</DialogTitle>
                </DialogHeader>
                        ...
                        <div className="space-y-6">
                            ...
                            {result.successCount > 0 && (
                                <div className="space-y-2">
                                    <h4 className="text-sm font-medium">Updated (up to 5)</h4>
                                    <div className="text-sm space-y-1">
                                        {result.tickets.filter(t => t.status === 'success').slice(0, 5).map((ticket, i) => (
                                            <div key={i} className="flex items-center justify-between p-2 bg-slate-50 rounded border">
                                                <span className="truncate flex-1 mr-2">{ticket.jiraKey}</span>
                                                {baseUrl ? (
                                                    <a
